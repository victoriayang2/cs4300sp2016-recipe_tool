...
def closest_recs_by_desc(rec_index_in, k = 10):
	sims = desc_rec_compressed.dot(desc_rec_compressed[rec_index_in,:])
	asort = np.argsort(-sims)[:k+1]
	return [(recipes[i]['name'],sims[i]/sims[asort[0]]) for i in asort[1:]]
...
# all_descs = []
# for r in recipes:
# 	all_descs.append(r['desc'])

# # with open("./data/recipes.pickle", "w") as f:
# #     pickle.dump(recipes, f)

# # Create recipe vectors by description text
# desc_vectorizer = TfidfVectorizer(stop_words='english', min_df=10, max_df=0.9)
# rec_by_desc = desc_vectorizer.fit_transform(all_descs)
# desc_by_rec = sparse.csr_matrix.transpose(rec_by_desc)

# print desc_by_rec.shape

# # Decompose desc_by_recipe matrix
# desc_words_compressed, _, desc_rec_compressed = sparse.linalg.svds(desc_by_rec, k=40)
# desc_rec_compressed = desc_rec_compressed.transpose()

# print "Desc: {}".format(desc_words_compressed.shape)
# print "Rec: {}".format(desc_rec_compressed.shape)

# desc_rec_compressed = normalize(desc_rec_compressed, axis = 1)
# # with open("./data/desc_rec_compressed.npy", "w") as f:
# # 	# desc_rec_compressed = np.load(f)
# # 	np.save(f, desc_rec_compressed)

# for i in random.sample(range(len(recipes)), 10):
#     print(recipes[i]['name'].encode("ascii", "ignore"))
#     for title, score in closest_recs_by_desc(i):
#         print("{}: {:.3f}".format(title.encode("ascii", "ignore"), score))
#     print
